...
export function Fish({ speed = 0.5, direction = 1, scale = 1, ...props }) {
  const group = useRef();
  const { scene, nodes, materials, animations } = useGLTF(fishScene);

  // FILTER BROKEN ANIMATION TRACKS
  const cleanedAnimations = animations.map((clip) => {
    const fixed = clip.clone();
    fixed.tracks = clip.tracks.filter((track) => {
      const nodeName = track.name.split(".")[0];
      return scene.getObjectByName(nodeName); // THIS FIXES ALL ERRORS
    });
    return fixed;
  });

  // Now pass the cleaned animations
  ...
  useFrame((state, delta) => {
    ...
    } else if (direction < 0 && group.current.position.x < initialX.current - range) {
      group.current.position.x = initialX.current + range;
    }
  });
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      <group name="Sketchfab_Scene">
        <group name="Sketchfab_model" rotation={[-Math.PI / 2, 0, 0]} scale={baseScale}>
          <group name="root">
